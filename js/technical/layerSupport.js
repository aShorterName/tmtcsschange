...
function setupLayer(layer){
	...
	if(layers[layer].boxShadowColor === undefined) layers[layer].boxShadowColor = tinycolor("#"+tinycolor(layers["p"].color).toHex().match(/.{1,2}/g).map(x=>Math.floor(parseInt(x,16)*0.6).toString(16)).map(x=>((x.length==1)?"0"+x:x)).join("")).toRgbString()
